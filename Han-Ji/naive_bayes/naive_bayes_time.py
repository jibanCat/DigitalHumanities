...
# pre-defined likelihood and prior
likelihood = {"年號" : 0.8, "數字": 0.6, "單位": 0.7, "季節" :0.6, "干支": 0.8, "姓氏":0.2, "不相干的字": 0.45}
time_prior = 0.3

# construct your bag-of-word via copy-paste the 年號 from MARKUS
...
unit = "年載月日初中末閏" # MARKUS 大概是用這行判斷單位
season = "春夏秋冬"
hour = {"辛巳","壬午","癸未","甲申","乙酉","丙戌","丁亥","戊子","己丑","庚寅","辛卯","壬辰","癸巳","甲午","乙未","丙申","丁酉","戊戌","己亥","庚子","辛丑","壬寅","癸卯","甲辰","乙巳","丙午","丁未","戊申","己酉","庚戌","辛亥","壬子","癸丑","甲寅","乙卯","丙辰","丁巳","戊午","己未","庚申","辛酉","壬戌","癸亥","甲子","乙丑","丙寅","丁卯","戊辰","己巳","庚午","辛未","壬申","癸酉","甲戌","乙亥","丙子","丁丑","戊寅","己卯","庚辰"}
...
def update_time_feature_vector(time_phrase, feature_vector, iterables, ):
    """
    Update the time feature vector based on a given time phrase and a dict of 
    time phrase feature iterables. The definiation of feature_vector refer to `calc_time_posterior`.
    """
    irrelevant_phrase = time_phrase

    for key, iterable in iterables.items():
        update_feature_vector(
            time_phrase, feature_vector, iterable, key
        )
...
def calc_time_posterior(phrase, iterables=iterables, likelihood=likelihood, time_prior=time_prior):
    """Calc the posterior of a time phrase based on a given feature iterables and 
    a pre-defined likelehood model and a time words frequency prior.
    Current feature for time: 
        {"年號", "數字", "單位", "季節", "干支", "姓氏", "不相干的字"}
    """
    feature_vector = {"年號" : 0,	"數字": 0, "單位": 0, "季節" :0, "干支": 0, "姓氏":0, "不相干的字": 0}
    feature_vector = update_time_feature_vector(
        phrase, feature_vector, iterables
    )
    return calc_posterior(feature_vector, likelihood, time_prior)